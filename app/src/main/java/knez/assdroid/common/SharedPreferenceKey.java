package knez.assdroid.common;

public final class SharedPreferenceKey {
    private SharedPreferenceKey() {}

    public static final String TAG_REPLACEMENT = "tag_replacement";
    public static final String SUBTITLE_LINE_SHOW_TIMINGS = "subtitle_line_show_timings";
    public static final String SUBTITLE_LINE_SHOW_STYLE_ACTOR = "subtitle_line_show_style_actor";
    public static final String SUBTITLE_LINE_SIMPLIFY_TAGS = "subtitle_line_simplify_tags";
    public static final String SUBTITLE_LINE_TEXT_SIZE_DP = "subtitle_line_text_size_dp";
    public static final String SUBTITLE_LINE_OTHER_SIZE_DP = "subtitle_line_other_size_dp";
    public static final String THEME = "theme";

}
